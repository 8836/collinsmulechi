...
export function TrackPage({ track }: { track: Track }) {
  return (
    <div className="space-y-16">
      <section>
        <p className="label-mono">{track.eyebrow}</p>
        <h1 className="mt-4 text-4xl font-bold leading-tight sm:text-5xl">{track.headline}</h1>
      ...
      <section>
        <h2 className="label-mono">What I do</h2>
      ...
      <section>
        <h2 className="label-mono">Relevant experience</h2>
        <ol className="mt-5 space-y-4">
          {track.experience.map((job) => (
            <li key={`${job.company}-${job.period}`} className="panel p-6">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
